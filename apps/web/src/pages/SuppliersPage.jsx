...
export default function SuppliersPage() {
  ...
  useEffect(() => {
    load().catch(() => setError('تعذر تحميل الموردين'));
  }, []);
  ...
  const save = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.error || 'تعذر حفظ المورد');
    }
  };
  ...
  const viewDetails = async (id) => {
    try {
      ...
      setError('تعذر تحميل تفاصيل المورد');
    }
  };
  ...
  return (
    <main className="container">
      ...
      <section className="card">
        <h2>{form.id ? 'تعديل مورد' : 'إضافة مورد'}</h2>
