...
export default function SalesPage() {
  ...
  const loadInitial = async () => {
    const [c, p, cash, inv] = await Promise.all([
      api.get('/customers'),
      api.get('/products'),
      api.get('/cash-accounts'),
      api.get('/sales')
    ]);
    setCustomers((c.data.data || []).filter((x) => x.is_active));
    setProducts((p.data.data || []).filter((x) => x.is_active));
    setCashAccounts(cash.data.data || []);
    setList(inv.data.data || []);
  };

  useEffect(() => {
    loadInitial().catch(() => setError('تعذر تحميل بيانات المبيعات'));
  }, []);
  ...
  const save = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.error || 'تعذر حفظ فاتورة البيع');
    }
  };
    ...
    <main className="container">
      ...
      <section className="card">
        ...
        <form onSubmit={save}>
          <div className="form-grid">
            ...
            <input type="number" min="0" step="0.01" placeholder="المبلغ المقبوض" value={form.paidAmount} onChange={(e) => setForm({ ...form, paidAmount: e.target.value })} />
          ...
          {form.items.map((item, idx) => (
            <div key={idx} className="form-grid" style={{ marginBottom: 8 }}>
              <select value={item.productId} onChange={(e) => setItem(idx, 'productId', e.target.value)} required>
                ...
                {products.map((p) => <option key={p.id} value={p.id}>{p.name_ar} (متاح {p.current_qty})</option>)}
              </select>
              ...
              <input type="number" min="0" step="0.01" placeholder="سعر البيع" value={item.unitPrice} onChange={(e) => setItem(idx, 'unitPrice', e.target.value)} required />
      ...
      <section className="card">
        <div className="header-actions" style={{ marginBottom: 10 }}>
          <input placeholder="بحث برقم الفاتورة أو العميل" value={search} onChange={(e) => setSearch(e.target.value)} />
      ...
      {details && (
        <section className="card">
          ...
          <p><strong>العميل:</strong> {details.customer_name || 'عميل نقدي'}</p>
          <p><strong>العملة:</strong> {details.currency} | <strong>الصرف:</strong> {details.exchange_rate}</p>
          <p><strong>الإجمالي:</strong> {details.total_original} | <strong>المقبوض:</strong> {details.received_original}</p>
          <p><strong>الحالة:</strong> {details.status}</p>
          <table className="table">
            <thead><tr><th>المنتج</th><th>الكمية</th><th>سعر البيع</th><th>الإجمالي</th><th>التكلفة</th><th>الربح</th></tr></thead>
            <tbody>
              {(details.items || []).map((it) => (
                <tr key={it.id}>
                  <td>{it.product_name}</td>
                  <td>{it.qty}</td>
                  <td>{it.unit_price_original}</td>
                  <td>{it.line_total_original}</td>
                  <td>{it.line_cogs_base}</td>
                  <td>{it.line_profit_base}</td>
                </tr>
              ))}
            </tbody>
          </table>
